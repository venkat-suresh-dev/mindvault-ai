...
export function ConversationSidebar({ conversations, selectedConversationId, isRefreshing, isBusy, error, onNewConversation, onSelectConversation, onRenameConversation, onDeleteConversation }: ConversationSidebarProps) {
  ...
  return <aside className="border-border bg-card h-fit rounded-2xl border p-4 shadow-sm" aria-labelledby="conversations-heading">
    ...
    <Button type="button" className="mt-4 w-full" onClick={onNewConversation} disabled={isBusy}><MessageSquarePlus />New chat</Button>
    ...
    <div className="mt-4 space-y-1" aria-live="polite">
      {conversations.length === 0 ? <EmptyConversationState /> : conversations.map((conversation) => {
        ...
        return <div key={conversation.id} className={cn("group rounded-r-lg border-l-2 border-transparent", isSelected && "border-primary bg-primary/10")}>
