        ...
        <aside className="border-border bg-card h-fit overflow-hidden rounded-2xl border shadow-sm">
          ...
          <div className="space-y-4 p-5">
            <div>
              ...
              <h1 className="mt-1 text-xl font-bold">{book.title}</h1>
              <p className="text-muted-foreground mt-1">{book.author}</p>
            </div>
            <dl className="text-sm">
              <Detail
                label="Voice persona"
                value={book.persona ?? "Default persona"}
              />
              <Detail
                label="Status"
                value={STATUS_LABELS[book.processingStatus]}
              />
              <Detail label="Uploaded" value={uploadedDate} />
              <Detail label="Pages" value="Not recorded" />
              <Detail label="Segments" value={String(book.totalSegments)} />
              <Detail
                label="Embedding model"
                value={book.embedding.model ?? "Not available"}
              />
              <Detail
                label="Dimensions"
                value={
                  book.embedding.dimensions
                    ? String(book.embedding.dimensions)
                    : "Not available"
                }
              />
            </dl>
        ...
        <div className="space-y-8">
          ...
          <section aria-labelledby="ai-features-heading">
            <div className="mb-4">
              ...
              <h2 id="ai-features-heading" className="mt-1 text-2xl font-bold">
                Upcoming AI features
              </h2>
              <p className="text-muted-foreground mt-2">
                These tools will build on the book processing already completed.
              </p>
            </div>
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {AI_FEATURES.map(([Icon, title, description]) => (
                <article
                  key={title}
                ...
                >
                  ...
                  <button
                    type="button"
                    disabled
                    className="border-border text-muted-foreground mt-4 rounded-lg border px-3 py-1.5 text-sm disabled:opacity-60"
                  >
                    Coming soon
                  </button>
                </article>
...
function Detail({ label, value }: { label: string; value: string }) {
  return (
    <div className="border-border flex items-start justify-between gap-4 border-b py-2 last:border-b-0">
      ...
      <dd className="max-w-36 text-right font-medium wrap-break-word">
        {value}
      </dd>
    </div>
  );
