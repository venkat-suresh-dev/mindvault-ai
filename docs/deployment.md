...
Knowledge lifecycle migrations must also create the `KnowledgeGeneration` unique generation index and its partial unique active-generation index over `{ userId, bookId, artifactType }`, the `KnowledgeGenerationBatch` scoped unique index over `{ clerkId, bookId, artifactType, generationId, batchIndex }`, the unique `AiUsage.idempotencyKey` index, and the unique `BlobCleanup` ownership lookup index over `{ bookId, clerkId, blobKey }`. The current Cron worker claims at most one job per invocation; it is not a continuously draining queue worker.
